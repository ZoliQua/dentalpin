...
| Event | Payload | When |
|---|---|---|
| `document.generated` | `{document_id, clinic_id, patient_id, document_type, title, created_by?}` | After successful PDF generation in `DocumentService.generate_pdf` (transactional, `db=db`) |
